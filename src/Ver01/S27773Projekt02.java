...
public class S27773Projekt02 { //klasa main
    ...
    public static void main ( String[] args ) {
        ...
        System.out.println ("2. Wczytaj zapis gry");
            ...
            switch (choice) {
                case 2 -> System.out.println("Brak implementacji");
                ...
                //wczytywanie gry
                // brak implementacji
                //FileHandler.loadBoard (fileName);
                //break;
                case 1 -> {
                    Board.startingBoard ();
    ...
    public void printBoard ( ) {
        for (int row = 0; row < 8; row++) {
            ...
            for (int col = 0; col < 8; col++) {
                ...
                if (takenBy (row, col) == null) {
                    System.out.print (printSquares (row, col) + " ");
                } else {
                    System.out.print (takenBy (row, col).getUnicode () + " ");
                }
            }
            System.out.println ();
        }
...
class FileHandler {
    ...
    public static Piece[] loadPieces ( String fileName ) {
        Piece[] pieces = new Piece[64];
        try (DataInputStream dis = new DataInputStream (new FileInputStream (fileName))) {
            for (int i = 0; i < pieces.length; i++) {
                byte b1 = dis.readByte ();
                byte b2 = dis.readByte ();
                int id = (b1 >> 5) & 0b00000111;
                int column = b1 & 0b00001111;
                int row = ((b1 & 0b11110000) << 3) | ((b2 >> 5) & 0b00001111);
                boolean isWhite = ((b2 >> 4) & 0b00000001) == 1;
                switch (id) {
                    case 0:
                        pieces[i] = new Pawn (column, row, byteToColor (isWhite));
                        break;
                    case 1:
                        pieces[i] = new King (column, row, byteToColor (isWhite));

                        break;
                    case 2:
                        pieces[i] = new Queen (column, row, byteToColor (isWhite));

                        break;
                    case 3:
                        pieces[i] = new Rook (column, row, byteToColor (isWhite));
                        break;
                    case 4:
                        pieces[i] = new Bishop (column, row, byteToColor (isWhite));
                        break;
                    case 5:
                        pieces[i] = new Knight (column, row, byteToColor (isWhite));
                        break;
                }
            }
...
class PlayerInput {
    ...
    static int ShowSelectedPieceIndex ( int startingCol, int startingRow ) {
