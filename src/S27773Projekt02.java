        ...
        System.out.println ("2. Wczytaj zapis gry");
            ...
            switch (choice) {
                case 2 ->  {
                    ...
                    try{
                        FileHandler.loadPieces(fileName);
                    }catch (IOException e) {
        ...
        while (gameOngoing) {
            ...
            switch (inGameChoice) {
                ...
                case 2:
                    ...
                    fileName = scanner.nextLine();
                    try{
                        FileHandler.saveBoard (fileName, pieces);
                    }catch ( IOException e) {
                        System.out.println("nie odnaleziono pliku");
                    }
...
class FileHandler {
    ...
    //Zapis planszy
    static public void saveBoard(String fileName, List<Piece> pieces) throws IOException {
        try (DataOutputStream outputStream = new DataOutputStream(new FileOutputStream(fileName))) {
            for (Piece piece : pieces) {
                byte savedPiece = encodePiece(piece);
                outputStream.writeByte(savedPiece);
            }
        }
    }
    ...
    // Zapis pojedynczej bierki
    public static byte encodePiece (Piece piece) {
        int id = piece.getId();
        int column = piece.getColumn();
        int row = piece.getRow();
        int color = piece.getColor() == 'w' ? 1 : 0;

        byte encodedPiece = (byte) id;
        encodedPiece |= (byte) (column << 3);
        encodedPiece |= (byte) (row << 7);
        encodedPiece |= (byte) (color << 6);

        return encodedPiece;
    }


    public static void loadPieces (String filename) throws IOException {


        try (DataInputStream inputStream = new DataInputStream(new FileInputStream(filename))) {
            while (inputStream.available() > 0) {
                byte encodedPiece = inputStream.readByte();
                decodePiece(encodedPiece);
    ...
    private static void decodePiece (byte encodedPiece) {
        int id = encodedPiece & 0b111;
        int column = (encodedPiece >> 3) & 0b1111;
        int row = (encodedPiece >> 7) & 0b1111;
        char cColor = (encodedPiece >> 6 & 0b1) == 1 ? 'w' : 'b';

        Colors color = cColor == 'w' ? Colors.w: Colors.b;

                switch (id) {
                    case 0:
                        S27773Projekt02.pieces.add(new Pawn(column, row,color));
                        break;
                    case 1:
                        S27773Projekt02.pieces.add(new King(column, row,color));
                        break;
                    case 2:
                        S27773Projekt02.pieces.add(new Queen(column, row,color));
                        break;
                    case 3:
                        S27773Projekt02.pieces.add( new Rook(column, row,color));
                        break;
                    case 4:
                        S27773Projekt02.pieces.add( new Bishop(column, row,color));
                        break;
                    case 5:
                        S27773Projekt02.pieces.add( new Knight(column, row,color));
                        break;
                }
...
class Pawn extends Piece {
    ...
    public boolean isValidMove ( int destinationCol, int destinationRow ) {
        ...
        //sprawdzamy czy wykonywane jest bicie w przelocie
       /* if(Board.takenBy(destinationRow,destinationCol) != null) {
            if (canDoEnPassant(destinationCol,destinationRow)) {
                return true;
            }
        }
*/
    ...
    public void move ( int desCol, int desRow ) {
        /*if (Board.takenBy(desRow,desCol) != null) {
            if (canDoEnPassant(desCol,desRow)) {
                doEnPassant(desCol,desRow);
            }else {
    ...
    public boolean canDoEnPassant (int destCol,int destRow) {
       ...
       //sprawdzamy id piona, który poruszył się dopiero o 2 pola
        ...
        //sprawdzamy, czy dopiero ruszył się o dwa pola
        if(!S27773Projekt02.pieces.get(idToCheck).isJustMoved2Squares())  {
            return false;
        }



        //ustalamy czy pion ma przed sobą piona przeciwnika
        if(S27773Projekt02.pieces.get(JustMovedPawnID).getColor() == 'w') {
            int threateningPieceRow = S27773Projekt02.pieces.get(JustMovedPawnID).getRow() + 1;
            int threateningPieceCol = S27773Projekt02.pieces.get(JustMovedPawnID).getColumn();
            if(Board.takenBy(threateningPieceRow,threateningPieceCol) != null ||
                    Board.takenBy(threateningPieceRow,threateningPieceCol).getColor() ==
                            S27773Projekt02.pieces.get(JustMovedPawnID).getColor()) {
                return false; //jeśli nie ma tam przeciwnika
            }
    ...
    void doEnPassant (int destinationCol, int destinationRow) {
        //sprawdzamy czy można wykonać EnPassant
        if (canDoEnPassant(destinationCol, destinationRow)) {


            //wykonanie bicia w przelocie
            {
                //określ nową lokalizacje (gracz podaje jedynie położenie zbijanego pionka)
                int newRow;
                int newCol;

                if (this.getColor() == 'w') {
                    newRow = destinationRow + 1;
                    newCol = destinationCol;
                } else {
                    newRow = destinationRow + 1;
                    newCol = destinationCol;
                }
                this.forceMove(newCol, newRow);

                GameLogic.killPiece(destinationRow, destinationCol);
