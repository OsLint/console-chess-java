...
class FileHandler {
    ...
    static public void saveBoard ( String fileName, List<Piece> pieces ) throws IOException {
        try (DataOutputStream outputStream = new DataOutputStream(new FileOutputStream(fileName))) {
            for (Piece piece : pieces) {
                byte savedPiece = savePeace(piece);
                outputStream.write(savedPiece);
            }
        }
    }
    ...
    public static byte savePeace ( Piece piece) throws IOException {
        int savedPiece = 0;

        // pierwsze 3 bity określają rodzaj figury lub piona ( 1 - król, 2- hetman, 3 - wieża, 4- goniec,
        // 5 - skoczek, 0 - pion)
        savedPiece |= piece.getId();

        // kolejne 4 bity określają pozycję poziomą
        savedPiece |= piece.getColumn() << 3;

        // kolejne 4 bity określają pozycję pionową
        savedPiece |= piece.getRow() << 7;

        // ostatni bit określa kolor bierki
        savedPiece |= piece.getColor() == 'w' ? 0b1 : 0b0;

        System.out.println(Integer.toBinaryString(savedPiece));
    ...
    public static void loadPieces(String fileName) throws IOException {
        InputStream load = new FileInputStream(fileName);
        List<Piece> pieces = new ArrayList<>();

        int readByte;
        while ((readByte = load.read()) != -1) {
            int row, column;
            boolean isWhite;

            isWhite = (readByte & 0b1) == 0b1;
            Colors color;
            if (isWhite) {
                color = Colors.w;
            }else {
            ...
            int id = readByte & 0b111;
            readByte >>= 3;

            column = readByte & 0b1111;
            readByte >>= 4;
            ...
            switch (id) {
                case 0:
                    pieces.add( new Pawn(column, row,color));
                    break;
                case 1:
                    pieces.add(new King(column, row,color));

                    break;
                case 2:
                    pieces.add(new Queen(column, row,color));

                    break;
                case 3:
                    pieces.add( new Rook(column, row,color));
                    break;
                case 4:
                    pieces.add( new Bishop(column, row,color));
                    break;
                case 5:
                    pieces.add( new Knight(column, row,color));
                    break;
            }
        }

        S27773Projekt02.pieces = pieces;
    }
